...
class StockReadingTest extends AnyFlatSpec with Matchers {
  ...
  "StockReader" should "correctly read simple Warden stocks" in {
    ...
    val expected = List(
      ...
      ItemStock("wolfhound", 2),
      ItemStock("dunne", 1),
      ItemStock("kivela", 1),
      ItemStock("spire", 1),
      ItemStock("gallant", 1),
      ItemStock("ressources", 32),
      ItemStock("shipping", 1),
      ItemStock("lariat", 1)
    )
